...
if (!defaultSecret) {
  throw new Error(
    "Missing STRIPE_WEBHOOK_SECRET in environment variables. Please add it to your .env file."
  );
}
...
class TestPayload {
  ...
  public constructor(timestamp = Date.now()) {
    ...
    const toSign = utf8.encode(
      `${this.id}.${this.timestamp}.${this.payload}`
    );

    this.signature = base64.encode(
      sha256.hmac(base64.decode(this.secret), toSign)
    );
...
test("partial signature throws error", () => {
  ...
  testPayload.header["svix-signature"] =
    testPayload.header["svix-signature"].slice(0, 8);
